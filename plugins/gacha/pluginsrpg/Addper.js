...
const handler = async (msg, { conn, args }) => {
  ...
  if (args.length < 5) {
    return conn.sendMessage(chatId, {
      text: `✳️ *Uso correcto:*\n.addper ⚔️Nombre ☠️Habilidad1 🐉Habilidad2 ImagenURL Precio\n\n📌 Ejemplo:\n.addper ⚔️Sung_Jin-Woo ☠️Sombra_Absoluta 🐉Dominio_Monarca https://cdn.catbox.click/890e3c01.jpeg 12000`
    }, { quoted: msg });
  }
  ...
  const masterpgPath = path.join(process.cwd(), "sukirpg.json");
  let data = {};
  if (fs.existsSync(masterpgPath)) {
    data = JSON.parse(fs.readFileSync(masterpgPath));
  }
  ...
  fs.writeFileSync(masterpgPath, JSON.stringify(data, null, 2));
