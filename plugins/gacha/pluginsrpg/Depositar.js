...
const handler = async (msg, { conn, args }) => {
  ...
  const masterpgPath = path.join(process.cwd(), "masterpg.json");
  let db = fs.existsSync(masterpgPath) ? JSON.parse(fs.readFileSync(masterpgPath)) : {};
  ...
  fs.writeFileSync(masterpgPath, JSON.stringify(db, null, 2));

  // 📷 Obtener avatar o fallback
  let avatarURL = "https://files.catbox.moe/tu4uir.jpeg";
  try {
